...
namespace BinanceApp
{
    public partial class VolumeInfo : Form
    {
        ...
        private async void VolumeInfo_Load(object sender, EventArgs e)
        {
            this.Show();
            await SetupCoinData();
        ...
        public void GetVolumes()
        {
            using (var client = new BinanceClient())
            {
                while (true)
                {
                    ...
                    {
                        var eighthourcandlesClient = client.GetKlines(pricer[0].ToString(), KlineInterval.FiveMinutes, startTime: DateTime.UtcNow.AddHours(-8), endTime: DateTime.UtcNow, limit: 100);
                        ...
                        var orderBook = client.GetOrderBook(pricer[0].ToString(), 10);
                        //BinanceOrderBook orderBook = await client.GetOrderBookAsync(pricer[0].ToString(), 10);
                        IOrderedEnumerable<BinanceOrderBookEntry> orderAsks = orderBook.Data.Asks.OrderByDescending(e => e.Quantity);
                        IOrderedEnumerable<BinanceOrderBookEntry> orderBids = orderBook.Data.Bids.OrderByDescending(e => e.Quantity);
                        decimal maxAsk = orderAsks.First().Price;
                        decimal maxBid = orderBids.First().Price;

                        var aggTrades = client.GetAggregatedTrades(pricer[0].ToString(), startTime: DateTime.UtcNow.AddMinutes(-2), endTime: DateTime.UtcNow, limit: 10);
                        //.Data.ToList()
                        ...
                        if (this.WindowState != FormWindowState.Minimized)
                        {
                            //Task.Run(() =>
                            //{
                            //coindatagridview.Invoke((MethodInvoker)delegate () { coindatagridview.DataSource = CoinData; });

                            //DataViewColours();
                            //backgroundWorkerView.DoWorkAsync();
            ...
            using (var client = new BinanceClient())
            {
                while (true)
                {
                    foreach (DataRow pricer in CoinData.Rows)
                    {
                        ...
                        var orderBook = await client.GetOrderBookAsync(pricer[0].ToString(), 10);
                        //BinanceOrderBook orderBook = await client.GetOrderBookAsync(pricer[0].ToString(), 10);
                        ...
                        decimal range = 100*(maxAsk- maxBid)/(maxAsk * maxBid);
                        ...
                        row[4] = PercentGive(candlearray.First().Volume, averagevolumeeighthours);
                        row[5] = PercentGive(fifteenminavg, averagevolumeeighthours);
                        row[7] = range; PercentGive(maxAsk,maxBid);
                        int i = 0;
                        while (i != 20)
                        {
                            if (candlearray.ElementAt(i).Close < candlearray.ElementAt(i + 1).Close) break;
        ...
        private void backgroundWorkerView_RunWorkerCompleted(object sender,RunWorkerCompletedEventArgs e)
        {
            ...
            //this.coindatagridview.Refresh();
            //coindatagridview.Update();
            DataViewColours();
        }
